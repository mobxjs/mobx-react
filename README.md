...
* `componentWillReact` won't fire when receiving new props or after `setState` calls (use `componentWillUpdate` instead) except version **4** and above since there `props` and `state` are treating as observable
